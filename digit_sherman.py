...
def open_canvas():
    B = Tkinter.Button(root, text ="Hit Me", command = save)
    B.pack()
    B2 = Tkinter.Button(root, text ="Clear", command =delete)
    B2.pack()
    B3 = Tkinter.Button(root, text ="Predict", command=prediction)
    B3.pack()
    cv.pack()
    root.mainloop()
...
class SampleListener(Leap.Listener):
    ...
    def on_frame(self, controller):
        ...
        if len(frame.hands)>0:
            ...
            if self.aveAmplitude<self.threshold:
                # Get hands
                ...
                for hand in frame.hands:
                    if hand.is_right:
                        ...
                        if(touch_distance<((thumb.width+index_finger.width)/2+15)):
                            ...
                            try:
                                thread.start_new_thread(draw_canvas,(hand_x+150,300-hand_y))
                            except:
                                continue
                            #print "draw start"
