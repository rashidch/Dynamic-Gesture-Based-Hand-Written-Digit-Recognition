...
h5f = h5py.File('C:/Users/rashi/Desktop/Elevator/svhn-multi-digit/data/SVHN_single_grey_9.h5', 'r')
...
def predict():
    ...
    save_dir = 'svhn-multi-digit/checkpoints/'
...
def process_image(image, label=None):
    ...
    image = cv2.resize(np.float32(image), dsize=(32,32),interpolation = cv2.INTER_AREA)
...
def predict_single_image(image, label=None):
    ...
    saver = tf.train.Saver()
    save_dir = 'C:/Users/rashi/Desktop/Elevator/svhn-multi-digit/checkpoints'
    # Create directory if it does not exist
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
