...
def predict_multi_image(image, label=None):
    image = cv2.resize(np.float32(image), dsize=(64,64),interpolation = cv2.INTER_AREA)
    image=subtract_mean(image)
    image=np.expand_dims(np.dot(image, [0.2989, 0.5870, 0.1140]), axis=3).astype(np.float32)
    print(image.shape)
    img = np.expand_dims(image, axis=0)
    pred = session.run(y_pred_cls, feed_dict={x:img , y_:[[1,10]] , p_keep_1: 1., p_keep_2: 1., p_keep_3: 1.})
    print(pred)
    return pred
