...
def conv_layer(input_tensor,    # The input or previous layer
    ...
    with tf.name_scope(layer_name):
        ...
        activations = tf.nn.conv2d(input_tensor, weights, [
                                   1, 1, 1, 1], 'SAME') + biases
        ...
        if pooling:
            ...
            activations = tf.nn.max_pool(
                activations, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')
...
# Block 1
conv_1 = conv_layer(drop_input, filter_size1, num_channels,
                    num_filters1, "conv_1", pooling=False)
conv_2 = conv_layer(conv_1, filter_size2, num_filters1,
                    num_filters2, "conv_2", pooling=True)
drop_block1 = tf.nn.dropout(conv_2, p_keep_2)  # Dropout

# Block 2
conv_3 = conv_layer(conv_2, filter_size3, num_filters2,
                    num_filters3, "conv_3", pooling=False)
conv_4 = conv_layer(conv_3, filter_size4, num_filters3,
                    num_filters4, "conv_4", pooling=True)
drop_block2 = tf.nn.dropout(conv_4, p_keep_2)  # Dropout

# Block 3
conv_5 = conv_layer(drop_block2, filter_size5, num_filters4,
                    num_filters5, "conv_5", pooling=False)
conv_6 = conv_layer(conv_5, filter_size6, num_filters5,
                    num_filters6, "conv_6", pooling=False)
conv_7 = conv_layer(conv_6, filter_size7, num_filters6,
                    num_filters7, "conv_7", pooling=True)
flat_tensor, num_activations = flatten_tensor(
    tf.nn.dropout(conv_7, p_keep_3))  # Dropout
...
def predict_multi_image(image, label=None):
    ...
    pred = session.run(y_pred_cls, feed_dict={x: img, y_: [
                       [1, 10]], p_keep_1: 1., p_keep_2: 1., p_keep_3: 1.})
    print(pred)
    return pred
# predict_multi_image('./15.jpg')
